...
def initialization(mesh_path,file):
    bpy.ops.wm.ply_import(filepath = mesh_path)  # import PLY
    model = bpy.data.objects[get_name(file)]
    scale = 0.003
    model.scale = (scale, scale, scale)  # scale
    model.rotation_euler = (0, math.pi*112/180, 0)
    ...
    node_group = model.modifiers[0].node_group
...
ply_path = "/home/jackzhu/Downloads/dataset_test/Rafa2_ply_400K/"

files = os.listdir(ply_path)
files.sort()
for file in files[:]:
    file_path = ply_path + file
    name = get_name(file) + '_train/'
    fp = "/home/jackzhu/Downloads/vsense/Rafa2_train/" + name

    if not os.path.exists(fp):
        os.makedirs(fp)
        initialization(file_path,file)
        out_data = {
        ...
        # cam.rotation_euler = (math.pi * 50.5 / 180, math.pi * 6.04 / 180, math.pi * 32.5 / 180)
